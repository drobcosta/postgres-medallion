   ...
   FROM ( SELECT d.id AS database_id,
           ...
           FROM data_catalog.tb_databases d
             ...
             LEFT JOIN pg_class hstlog_tbl ON hstlog_tbl.relkind = 'r'::"char" AND hstlog_tbl.relnamespace = hstlog_sch.oid AND hstlog_tbl.relname ~~ concat(t.name, '%') AND hstlog_tbl.relname ~~ '%_auditlog'::text
			          LEFT JOIN pg_class hstlog_tbl_insert ON hstlog_tbl_insert.relkind = 'r'::"char" AND hstlog_tbl_insert.relnamespace = hstlog_sch.oid AND hstlog_tbl_insert.relname = concat(t.name,'_auditlog_insert')::text
             LEFT JOIN pg_class hstlog_tbl_update ON hstlog_tbl_update.relkind = 'r'::"char" AND hstlog_tbl_update.relnamespace = hstlog_sch.oid AND hstlog_tbl_update.relname = concat(t.name,'_auditlog_update')::text
             LEFT JOIN pg_class hstlog_tbl_delete ON hstlog_tbl_delete.relkind = 'r'::"char" AND hstlog_tbl_delete.relnamespace = hstlog_sch.oid AND hstlog_tbl_delete.relname = concat(t.name,'_auditlog_delete')::text) unnamed_subquery
