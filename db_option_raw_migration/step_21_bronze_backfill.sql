...
BEGIN
	...
	IF p_databases_id IS NOT NULL AND p_schemas_id IS NOT NULL AND p_tables_id IS NOT NULL AND p_columns_id IS NULL THEN
		...
		FOR v_record IN
			...
			LEFT JOIN data_catalog.bronze_payload_control blc
		...
		LOOP
			...
			IF v_record.update_done IS NULL OR v_record.update_done IS FALSE THEN
		        ...
		        IF v_cmd IS NOT NULL THEN
				  ...
				  IF v_update_timestamp >= v_target_timestamp THEN
		            UPDATE data_catalog.bronze_backfill_control SET
		            update_done = true
		            WHERE id = v_record.bronze_layer_control_id;
				  END IF;
...
BEGIN
	...
	-- Se SIM, insert into data_catalog.bronze_payload_control
	...
	, bronze_payload_control_add AS (
		INSERT INTO data_catalog.bronze_payload_control(
			tb_databases_id,
			tb_schemas_id,
			tb_tables_id,
			insert_timestamp,
	...
	SELECT COALESCE((
		...
		FROM bronze_payload_control_add
	), false)
